...
if y_plot1[:] != '' and y_plot2[:] != '' and y_plot3[:] != '' and y_plot4[:] != '':
    ...
    plt.legend(['Популярность', 'Интерес к продолжению', 'Разочарование', 'Заинтересованность'], loc=0, fontsize=5)
    plt.show()
else:
    y_empty = 'Мы не смогли построить график. Данные за 2022-2017 гг. отсутствуют. '
...
if len(y_plot) != 0:
    ...
    #         autopct= lambda p: '{:.1f} тыс.'.format(p * sum(y_plot) / 100), pctdistance=0.65)
    # plt.title('Средний заработок по году, тыс. рублей', color='b')
    plt.show()
